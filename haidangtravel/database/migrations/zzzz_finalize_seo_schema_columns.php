...
return new class extends Migration
{
    public function up(): void
    {
        if (Schema::hasTable('content_clusters')) {
            Schema::table('content_clusters', function (Blueprint $table): void {
                if (! Schema::hasColumn('content_clusters', 'business_value')) {
                    $table->unsignedTinyInteger('business_value')->default(5)->after('target_page_type');
                }

                if (! Schema::hasColumn('content_clusters', 'priority_score')) {
                    $table->unsignedTinyInteger('priority_score')->default(50)->after('business_value');
                }
        ...
        if (Schema::hasTable('seo_pages')) {
            Schema::table('seo_pages', function (Blueprint $table): void {
                if (! Schema::hasColumn('seo_pages', 'page_type')) {
                    $table->string('page_type')->default('service')->after('content_cluster_id');
                }
        ...
        if (Schema::hasTable('seo_links')) {
            Schema::table('seo_links', function (Blueprint $table): void {
                ...
                if (! Schema::hasColumn('seo_links', 'priority')) {
                    $table->unsignedTinyInteger('priority')->default(50)->after('link_type');
                }
