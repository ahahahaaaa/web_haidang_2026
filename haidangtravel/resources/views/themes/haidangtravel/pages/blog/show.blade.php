...
@section('content')
    @php
        $coverMedia = \App\Support\FrontsiteMedia::responsiveUrls($post, 'cover', 'cover_image_url');
        ...
        $faqItems = collect($post->faq_items ?? [])
            ...
            ->values();
        $blogCategoryPathLabel = $blogCategoryTrail->isNotEmpty()
            ? \App\Support\ContentCategoryTree::pathLabel($post->category)
            : null;
    ...
    <section class="px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl space-y-6">
            <div class="theme-panel frontsite-text-reveal p-6 sm:p-8" data-reveal="panel">
                ...
                @if ($tocItems !== [])
                    <nav class="mt-6" aria-label="Mục lục bài viết">
                        <ol class="space-y-3">
                ...
                <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div class="rounded-sm bg-slate-50 px-5 py-4">
                        ...
                        @if ($blogCategoryTrail->isNotEmpty())
                            ...
                            @if ($blogCategoryPathLabel)
                                <p class="mt-3 text-sm leading-6 text-slate-500">{{ $blogCategoryPathLabel }}</p>
                            @endif
                        @else
