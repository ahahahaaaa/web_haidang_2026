...
@section('content')
    ...
    <section class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section class="theme-panel mb-10 p-6 sm:p-8" data-ai-summary>
            <div class="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">
                <span class="rounded-sm bg-[color:var(--color-primary-soft)] px-3 py-1 text-primary">
                    {{ $selectedServiceCategory?->name ?: 'Dịch vụ du lịch' }}
                </span>
                <span>{{ $services->count() }} dịch vụ đang hiển thị</span>
            </div>

            <div class="mt-6 grid gap-4 md:grid-cols-2">
                <div class="rounded-sm bg-slate-50 px-5 py-4 text-sm leading-7 text-slate-600">
                    {{ $selectedServiceCategory?->description ?: ($landing?->hero_excerpt ?: 'Danh sách dịch vụ giúp người dùng đi thẳng vào đúng nhóm nhu cầu trước khi mở trang chi tiết hoặc gửi yêu cầu tư vấn.') }}
                </div>
                <div class="rounded-sm bg-slate-50 px-5 py-4 text-sm leading-7 text-slate-600">
                    {{ $selectedServiceCategory ? 'Trang danh mục này dùng URL riêng để gom nhóm dịch vụ cùng intent, hỗ trợ internal linking và canonical rõ ràng hơn cho frontsite.' : 'Trang dịch vụ tổng giữ vai trò hub để điều hướng sang từng nhóm dịch vụ và từng service detail có khả năng chuyển đổi cao hơn.' }}
                </div>
            </div>
        </section>
