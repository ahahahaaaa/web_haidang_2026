...
@section('content')
    ...
    <section class="px-4 py-9 sm:px-6 lg:px-8 lg:py-11">
        <div class="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.05fr_0.95fr]">
            <div class="space-y-8">
                <article class="theme-panel frontsite-text-reveal p-6 sm:p-8 lg:p-10" data-reveal="copy" data-ai-summary>
                    <div class="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">
                        @if ($service->category)
                            <a href="{{ $serviceCategoryUrl }}" class="rounded-sm bg-[color:var(--color-primary-soft)] px-3 py-1 text-primary transition hover:brightness-95">
                                {{ $service->category->name }}
                            </a>
                        @endif
                        <a href="{{ route('services.index') }}" class="rounded-sm border border-slate-200 px-3 py-1 transition hover:border-orange-200 hover:text-primary">
                            Toàn bộ dịch vụ
                        </a>
                    ...
                    <div class="mt-6 grid gap-4 md:grid-cols-2">
                        <div class="rounded-sm bg-slate-50 px-5 py-4 text-sm leading-7 text-slate-600">
                            {{ $service->excerpt ?: 'Trang này tập trung giải thích rõ phạm vi dịch vụ, điểm nổi bật và cách nhận tư vấn phù hợp.' }}
                        </div>
                        <div class="rounded-sm bg-slate-50 px-5 py-4 text-sm leading-7 text-slate-600">
                            {{ $service->price_note ?: 'Nếu cần báo giá, phạm vi xử lý hoặc timeline, hãy để lại yêu cầu để đội ngũ tư vấn theo đúng nhu cầu.' }}
                        </div>
                    </div>
                        ...
                        <div class="mt-6">
                            ...
                            <div class="mt-4 flex flex-wrap gap-3">
                                @foreach ($relatedQuestions as $question)
                                    <span class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600">{{ $question }}</span>
                                @endforeach
                            </div>
                        </div>
                    @endif
