@php
    ...
    $normalizeMenuLabel = fn (?string $label) => \Illuminate\Support\Str::of((string) $label)->ascii()->lower()->squish()->value();
    $normalizeMenuUrl = fn (?string $url) => '/'.ltrim((string) parse_url((string) $url, PHP_URL_PATH), '/');
    $legacyHeaderKeywords = [
        'xay dung',
        'thi cong',
        'du an',
        'du-toan',
    ...
    $isLegacyHeaderItem = function (?string $label, ?string $url = null) use ($legacyHeaderKeywords, $normalizeMenuLabel, $normalizeMenuUrl): bool {
        $normalizedLabel = $normalizeMenuLabel($label);
        $normalizedUrl = $normalizeMenuLabel($normalizeMenuUrl($url));

        foreach ($legacyHeaderKeywords as $keyword) {
            if (\Illuminate\Support\Str::contains($normalizedLabel, $keyword) || \Illuminate\Support\Str::contains($normalizedUrl, $keyword)) {
                return true;
            }
        }

        return in_array($normalizeMenuUrl($url), ['/du-an', '/du-toan', '/bao-gia', '/kien-truc', '/noi-that', '/thi-cong'], true);
    };
    $menuItems = collect($headerMenu ?? [])
        ->filter(fn ($item) => filled($item->label ?? null) && filled($item->url ?? null))
        ->reject(fn ($item) => $isLegacyHeaderItem($item->label ?? null, $item->url ?? null))
        ->values();
