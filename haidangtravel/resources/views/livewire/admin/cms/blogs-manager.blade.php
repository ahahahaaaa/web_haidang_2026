<div class="space-y-6">
    <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
            <h1 class="text-3xl font-semibold text-zinc-900 dark:text-white">Quản lý blog</h1>
            <p class="text-sm text-zinc-500 dark:text-zinc-400">Quản lý bài viết, danh mục blog, SEO và cover image bằng Livewire.</p>
        </div>
    ...
    <div class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <div class="space-y-6">
            <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Danh sách bài viết</h2>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">Lọc theo tên và trạng thái xuất bản.</p>
                    </div>

                    <button type="button" wire:click="createPost" class="rounded-2xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white dark:bg-white dark:text-zinc-900">
                        Bài viết mới
                    </button>
                </div>
                ...
                <div class="space-y-3">
                    @foreach ($posts as $post)
                        <div wire:key="blog-post-{{ $post->id }}" class="rounded-3xl border border-zinc-200 bg-zinc-50 p-4 transition hover:border-red-300 dark:border-zinc-700 dark:bg-zinc-800/70">
                            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                <div>
                                    <button type="button" wire:click="editPost({{ $post->id }})" class="text-left text-base font-semibold text-zinc-900 transition hover:text-red-600 dark:text-white">
                                        {{ $post->title }}
                                    </button>
                                    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
                                        <span class="rounded-full bg-white px-3 py-1 font-semibold text-zinc-600 ring-1 ring-zinc-200 dark:bg-zinc-900 dark:text-zinc-300 dark:ring-zinc-700">{{ $post->status }}</span>
                                        @if ($post->category)
                                            <span class="rounded-full bg-red-50 px-3 py-1 font-semibold text-red-700 dark:bg-red-500/10 dark:text-red-300">{{ $post->category->name }}</span>
                                        @endif
                                        <span class="text-zinc-500 dark:text-zinc-400">{{ $post->author_name }}</span>
                                    </div>
                                </div>

                                <button type="button" wire:click="deletePost({{ $post->id }})" class="rounded-2xl border border-red-200 px-4 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50 dark:border-red-500/30 dark:text-red-300 dark:hover:bg-red-500/10">
                                    Xóa
                                </button>
                            </div>
                        </div>
            ...
            <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="mb-5">
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Danh mục blog</h2>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">Quản lý danh mục để lọc bài viết và SEO structure.</p>
                </div>

                <div class="mb-4 space-y-3">
                    @foreach ($categories as $category)
                        <div wire:key="blog-category-{{ $category->id }}" class="flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-800/70">
                            <div>
                ...
                <form wire:submit="saveCategory" class="grid gap-4 rounded-3xl bg-zinc-50 p-5 dark:bg-zinc-800/70 md:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <input type="text" wire:model.defer="categoryForm.name" class="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white">
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Slug</label>
                        <input type="text" wire:model.defer="categoryForm.slug" class="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" wire:model.defer="categoryForm.sort_order" class="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white">
                    </div>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <h2 class="mb-5 text-lg font-semibold text-zinc-900 dark:text-white">Biên tập bài viết</h2>

            <form wire:submit="savePost" class="space-y-5">
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Danh muc</label>
