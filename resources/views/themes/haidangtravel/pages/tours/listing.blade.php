...
@section('content')
    @php
        ...
        $countryClusterDestinations = collect($countryClusterDestinations ?? []);
        $hideListingContextPanel = (bool) ($hideListingContextPanel ?? false);
        $showToursOnPage = (bool) ($showToursOnPage ?? true);
        $showBlogsOnPage = (bool) ($showBlogsOnPage ?? false);
        $destinationBlogPosts = collect($destinationBlogPosts ?? []);
        ...
        $hasTaxonomyContext = $fixedContextMap->has('Danh mục') || $fixedContextMap->has('Điểm đến') || $fixedContextMap->has('Vùng miền');
        $listingSectionTitle = match (true) {
            $hasTaxonomyContext => $pageTitle,
        ...
        $showGeneratedListingHeading = ! $hasTaxonomyContext;
        $showDestinationToursBeforeIntro = $fixedContextMap->has('Điểm đến') && $showToursOnPage;
    ...
    @if ($countryClusterDestinations->isNotEmpty())
        <section class="px-4 pb-6 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-7xl">
                <div class="frontsite-text-reveal flex flex-wrap items-center gap-3" data-reveal="meta">
                    <span class="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Điểm đến thuộc quốc gia</span>
                    @foreach ($countryClusterDestinations as $destination)
                        <a href="{{ route('destinations.show', $destination) }}" class="inline-flex items-center rounded-sm border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-orange-200 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30">
                            {{ $destination->name }}
                        </a>
                    @endforeach
                </div>
    ...
    <section class="px-4 pb-10 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl space-y-6">
            <div id="tour-list"  class="space-y-6">
                @if ($showToursOnPage)
                    ...
                    @includeWhen(! empty($tourVoucherCta ?? null), 'themes.haidangtravel.partials.tour-voucher-cta-button', [
                        'containerClasses' => 'frontsite-text-reveal flex justify-center py-2',
                        'voucherCta' => $tourVoucherCta ?? null,
                    ])
                @endif
            </div>
            @unless ($hideListingContextPanel)
                @if (($showGeneratedListingHeading || $pageContent) && ! $showDestinationToursBeforeIntro)
                    <div class="space-y-6">
                        @if ($showGeneratedListingHeading)
                        ...
                        @if ($pageContent)
                            <div class="theme-panel frontsite-text-reveal overflow-hidden p-0" data-reveal="copy">
                                @include('themes.haidangtravel.partials.expandable-rich-text', [
                                    ...
                                    'expandedByDefault' => $fixedContextMap->has('Điểm đến') || $fixedContextMap->has('Quốc gia'),
            ...
            <div class="space-y-6">
                ...
                @if ($showDestinationToursBeforeIntro && ! $hideListingContextPanel && ($showGeneratedListingHeading || $pageContent))
                    <div class="space-y-6">
                        @if ($showGeneratedListingHeading)
                            @include('themes.haidangtravel.partials.section-heading', [
                                'title' => $listingSectionTitle,
                ...
                @if ($showBlogsOnPage && $destinationBlogPosts->isNotEmpty())
                    <section id="destination-blog-list" class="space-y-5">
                        @include('themes.haidangtravel.partials.section-heading', [
                            'eyebrow' => 'Cẩm nang điểm đến',
                            'title' => 'Cẩm nang du lịch '.$pageTitle,
                            'description' => 'Các bài viết liên quan giúp chuẩn bị lịch trình, ngân sách và thông tin cần biết trước chuyến đi.',
                            'width' => 'max-w-none',
                        ])

                        <div class="{{ \App\Support\FrontsiteCardGrid::classes() }}">
                            @foreach ($destinationBlogPosts as $post)
