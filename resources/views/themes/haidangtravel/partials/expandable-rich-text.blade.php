@php
    ...
    $expandedByDefault = (bool) ($expandedByDefault ?? false);
...
@if ($introText !== '' || $contentHtml !== '')
    <div
        ...
        data-expanded="{{ $expandedByDefault ? 'true' : 'false' }}"
        data-collapsed-height-mobile="{{ $mobileCollapsedHeight }}"
        data-collapsed-height-desktop="{{ $desktopCollapsedHeight }}"
        style="--tour-details-collapsed-height-mobile: {{ $mobileCollapsedHeight }}px; --tour-details-collapsed-height-desktop: {{ $desktopCollapsedHeight }}px;"
    >
        <div class="relative">
            <div
                id="{{ $expandableId }}"
                ...
                @if ($expandedByDefault) style="max-height: none;" @endif
            >
                @if ($introText !== '')
