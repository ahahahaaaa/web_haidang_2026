@php
    ...
    $formVariant = $formVariant ?? 'default';
    $isVoucherForm = $formVariant === 'voucher';
    $submitLabel = $submitLabel ?? 'Gửi yêu cầu';
    $voucherCampaignSlug = $voucherCampaignSlug ?? null;
    $voucherVariant = $voucherVariant ?? ($abVariant ?? null);
    $inquiryTypeOptions = [
        'travel' => 'Du lịch',
...
<input type="hidden" name="voucher_campaign_slug" value="{{ old('voucher_campaign_slug', $voucherCampaignSlug) }}" data-travel-inquiry-voucher-campaign-input>
<input type="hidden" name="voucher_variant" value="{{ old('voucher_variant', old('ab_variant', $voucherVariant)) }}" data-travel-inquiry-voucher-variant-input data-travel-inquiry-ab-variant-input>
@if ($isVoucherForm)
...
<div class="grid gap-5 md:grid-cols-3">
    ...
    @if ($isVoucherForm)
        <label class="space-y-2">
            <span class="text-sm font-semibold text-slate-900">Bạn dự kiến đi đâu? <span class="text-danger">*</span></span>
            <input
                type="text"
                name="expected_destination"
                aria-invalid="{{ $submissionAttempted && $errors->has('expected_destination') ? 'true' : 'false' }}"
                value="{{ old('expected_destination') }}"
                placeholder="Nhập điểm đến hoặc khu vực muốn đi"
                class="frontsite-form-control min-h-13 w-full rounded-[1rem] border border-slate-200 bg-white px-4 py-3 text-base text-slate-900 outline-none transition focus:border-primary {{ $submissionAttempted && $errors->has('expected_destination') ? 'is-invalid' : '' }}"
            >
            <p class="frontsite-form-field-error {{ $submissionAttempted && $errors->has('expected_destination') ? 'text-danger' : 'hidden text-danger' }}" data-frontsite-field-error="expected_destination">
                @if ($submissionAttempted)
                    {{ $errors->first('expected_destination') }}
                @endif
            </p>
        </label>

        <label class="space-y-2">
            <span class="text-sm font-semibold text-slate-900">Bạn dự kiến đi thời gian nào? <span class="text-danger">*</span></span>
            <input
                type="text"
                name="expected_time"
                aria-invalid="{{ $submissionAttempted && $errors->has('expected_time') ? 'true' : 'false' }}"
                value="{{ old('expected_time') }}"
                placeholder="Nhập ngày, tháng hoặc khoảng thời gian dự kiến"
                class="frontsite-form-control min-h-13 w-full rounded-[1rem] border border-slate-200 bg-white px-4 py-3 text-base text-slate-900 outline-none transition focus:border-primary {{ $submissionAttempted && $errors->has('expected_time') ? 'is-invalid' : '' }}"
            >
            <p class="frontsite-form-field-error {{ $submissionAttempted && $errors->has('expected_time') ? 'text-danger' : 'hidden text-danger' }}" data-frontsite-field-error="expected_time">
                @if ($submissionAttempted)
                    {{ $errors->first('expected_time') }}
                @endif
            </p>
        </label>
    @else
