...
<html lang="vi">
    ...
    <body class="frontsite-theme bg-[linear-gradient(180deg,var(--color-bg)_0%,#ffffff_18%,var(--color-bg-soft)_100%)] font-body text-slate-900">
        ...
        @php
            $afterHeaderHtml = \App\Support\SitewideHtmlSnippets::afterHeaderHtml($siteSettings->after_header_html);
        @endphp
        @if (filled($afterHeaderHtml))
            {!! $afterHeaderHtml !!}
        @endif
        ...
        @include('themes.haidangtravel.partials.recaptcha-v3-script')
        @php
            $endBodyHtml = \App\Support\SitewideHtmlSnippets::endBodyHtml($siteSettings->end_body_html);
        @endphp
        @if (filled($endBodyHtml))
            {!! $endBodyHtml !!}
        @endif
    </body>
</html>
