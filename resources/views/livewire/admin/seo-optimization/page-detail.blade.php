<div class="space-y-4 pb-24">
    ...
    <section class="space-y-3 rounded-3xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        @if ($latestAudit)
            <div class="flex flex-wrap items-center gap-3 text-sm text-zinc-700 dark:text-zinc-200"><span class="text-2xl font-semibold">{{ $this->auditScoreLabel($latestAudit->score) }}</span><span>{{ $this->auditGradeStatusLabel($latestAudit->grade, $latestAudit->status) }}</span><span class="text-xs text-zinc-500">{{ $latestAudit->created_at?->format('d/m/Y H:i') }}</span></div>
            @if ($latestAudit->score === null)
                <p class="text-sm font-medium text-amber-700 dark:text-amber-300">{{ $this->auditCoverageLabel($latestAudit->report ?? []) }}</p>
            @endif
    ...
    <section class="space-y-3 rounded-3xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="text-xs text-zinc-500 dark:text-zinc-400"><tr><th class="py-2">Đề xuất</th><th class="px-3 py-2">Điểm trước / sau</th><th class="px-3 py-2">Trạng thái</th><th class="px-3 py-2">Ngày tạo</th></tr></thead><tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
            @forelse ($proposals as $proposal)
                <tr wire:key="proposal-{{ $proposal->id }}"><td class="py-3"><a wire:navigate href="{{ route('admin.seo-optimization.proposals.show', $proposal) }}" class="font-medium text-teal-700 hover:underline dark:text-teal-300">Xem đề xuất {{ $proposal->id }}</a></td><td class="px-3 py-3">@include('livewire.admin.seo-optimization.partials.score-comparison', ['proposal' => $proposal, 'compact' => true])</td><td class="px-3 py-3 text-zinc-700 dark:text-zinc-200">{{ $this->statusLabel($proposal->status) }}</td><td class="whitespace-nowrap px-3 py-3 text-zinc-500 dark:text-zinc-400">{{ $proposal->created_at?->format('d/m/Y H:i') }}</td></tr>
            @empty
                <tr><td colspan="4" class="py-6 text-center text-zinc-500 dark:text-zinc-400">Chưa có đề xuất. Hãy lưu brief, kiểm tra SEO và đưa URL vào hàng chờ.</td></tr>
            @endforelse
        </tbody></table></div>
