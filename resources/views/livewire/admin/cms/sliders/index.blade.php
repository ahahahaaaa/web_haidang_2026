            ...
            <table class="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-800">
                ...
                <tbody class="divide-y divide-zinc-100 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                    @forelse ($sliders as $slider)
                        @php
                            $locationLabel = $slider->location
                                ? (($sliderLocationOptions[$slider->location]['label'] ?? null) ?: $slider->location)
                                : null;
                        @endphp
                        <tr wire:key="slider-index-row-{{ $slider->id }}" class="align-top">
                            ...
                            <td class="px-4 py-4 text-zinc-500 dark:text-zinc-400">
                                @if ($locationLabel)
                                    <span class="font-medium text-zinc-700 dark:text-zinc-200">{{ $locationLabel }}</span>
                                    @if ($locationLabel !== $slider->location)
                                        <span class="mt-1 block text-xs">{{ $slider->location }}</span>
                                    @endif
                                @else
                                    Chưa có
                                @endif
