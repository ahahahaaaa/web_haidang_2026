@php
    ...
    $saleDefaults = \App\Support\Admin\AdminNavigationRegistry::defaultSalePermissions();
        ...
        <section class="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            @can('admin.accounts.edit')
                <form wire:submit="save" class="space-y-5 pb-32 md:pb-6" data-admin-feedback-form data-admin-loading-text="Đang lưu tài khoản...">
                    ...
                    <section class="space-y-4 rounded-3xl border border-zinc-200 bg-zinc-50/70 p-4 dark:border-zinc-800 dark:bg-zinc-950/40">
                        ...
                        <div class="grid gap-4 md:grid-cols-3">
                            <label class="flex cursor-pointer items-start gap-3 rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                                <input type="radio" wire:model.live="form.role_type" value="admin" class="mt-1 rounded border-zinc-300 text-teal-600">
                                <span>
                            ...
                            <label class="flex cursor-pointer items-start gap-3 rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                                <input type="radio" wire:model.live="form.role_type" value="content" class="mt-1 rounded border-zinc-300 text-teal-600">
                                <span>
                            ...
                            <label class="flex cursor-pointer items-start gap-3 rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                                <input type="radio" wire:model.live="form.role_type" value="sale" class="mt-1 rounded border-zinc-300 text-teal-600">
                                <span>
                    ...
                    <section class="space-y-4 rounded-3xl border border-zinc-200 bg-zinc-50/70 p-4 dark:border-zinc-800 dark:bg-zinc-950/40">
                        <div>
                            <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Quản lý quyền</h3>
                            ...
                            @elseif (($form['role_type'] ?? 'content') === 'sale')
                                <p class="text-sm text-zinc-500 dark:text-zinc-400">Sale dùng bộ quyền tour cố định; không thể cấp quyền quản trị tài khoản hoặc các module khác.</p>
                            @else
                                <p class="text-sm text-zinc-500 dark:text-zinc-400">Admin có toàn quyền trên toàn bộ sidebar và action của CMS.</p>
                            @endif
                        </div>
                        ...
                        @if (($form['role_type'] ?? 'content') === 'content')
                            ...
                            <div class="space-y-4">
                                @foreach ($accountGroups as $group)
                                    <div wire:key="account-permission-group-{{ $group['key'] }}" class="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                                        <div class="mb-3">
                                        ...
                                        <div class="grid gap-3 md:grid-cols-2">
                                            @foreach ($group['actions'] as $action)
                                                ...
                                                @endphp
                                                <label wire:key="account-permission-{{ $group['key'] }}-{{ $loop->index }}" class="flex items-start gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm dark:border-zinc-800 dark:bg-zinc-950/40">
                                                    @if ($isDefault)
                                                        <input type="checkbox" checked disabled class="mt-1 rounded border-zinc-300 text-teal-600">
                                                    @else
                                                        <input type="checkbox" value="{{ $action['permission'] }}" wire:model.defer="form.extra_permissions" class="mt-1 rounded border-zinc-300 text-teal-600">
                                                    @endif
                                                    <span>
                        ...
                        @elseif (($form['role_type'] ?? 'content') === 'sale')
                            <div class="rounded-2xl border border-dashed border-zinc-300 px-4 py-3 text-sm text-zinc-500 dark:border-zinc-700 dark:text-zinc-400">
                                Quyền cố định:
                                <span class="font-medium">{{ implode(', ', array_values(array_filter($saleDefaults, fn ($permission) => $permission !== 'access admin panel'))) }}</span>
                            </div>
                        @else
                            <div class="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">
                                Toàn quyền được đồng bộ tự động theo vai trò Admin; không cần chọn từng quyền riêng lẻ.
                            </div>
                        @endif
                    </section>
