<div class="space-y-6">
    ...
    >
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            @php
                $apiCheckSucceeded = data_get($customerLoyaltyApiCheckResult, 'status') === 'success';
                $apiTokenExpired = (bool) data_get($customerLoyaltyApiStatus, 'token_is_expired');
            @endphp

            <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">API quà tặng CRM</h2>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">Cấu hình cổng API frontstore dùng cho trang kiểm tra điểm và tạo phiếu đổi quà chờ duyệt.</p>
                </div>

                <button
                    type="button"
                    wire:click="checkCustomerLoyaltyApi"
                    wire:loading.attr="disabled"
                    wire:target="checkCustomerLoyaltyApi"
                    class="inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 px-5 py-3 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-70 dark:border-red-500/30 dark:text-red-300 dark:hover:bg-red-500/10"
                >
                    <i class="fa-solid fa-plug-circle-check" wire:loading.remove wire:target="checkCustomerLoyaltyApi"></i>
                    <i class="fa-solid fa-spinner animate-spin" wire:loading wire:target="checkCustomerLoyaltyApi"></i>
                    <span wire:loading.remove wire:target="checkCustomerLoyaltyApi">Kiểm tra đăng nhập API</span>
                    <span wire:loading wire:target="checkCustomerLoyaltyApi">Đang kiểm tra...</span>
                </button>
            </div>

            <div class="grid gap-4 md:grid-cols-2">
                <div class="space-y-2 md:col-span-2">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Base URL API</label>
                    <input type="url" wire:model.defer="form.customer_loyalty_api_base_url" placeholder="https://haidangtravel.local:8443/api" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                    <p class="text-xs text-zinc-500 dark:text-zinc-400">Ví dụ môi trường test: <code>https://haidangtravel.local:8443/api</code>. Sau này đổi sang cổng đối tác cung cấp tại đây.</p>
                    @error('form.customer_loyalty_api_base_url') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>

                <div class="space-y-2">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Tài khoản đăng nhập API</label>
                    <input type="text" wire:model.defer="form.customer_loyalty_api_username" autocomplete="off" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                    @error('form.customer_loyalty_api_username') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>

                <div class="space-y-2">
                    <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Mật khẩu API</label>
                    <input type="password" wire:model.defer="form.customer_loyalty_api_password" autocomplete="new-password" placeholder="Để trống nếu không đổi" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                    <p class="text-xs text-zinc-500 dark:text-zinc-400">Mật khẩu được lưu dạng mã hóa. Hãy lưu cấu hình trước khi bấm kiểm tra đăng nhập API.</p>
                    @error('form.customer_loyalty_api_password') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
            </div>

            @error('customerLoyaltyApi')
                <p class="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300">{{ $message }}</p>
            @enderror

            <div class="mt-5 grid gap-3 md:grid-cols-4">
                <div class="rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-950/50">
                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Cấu hình</p>
                    <p class="mt-2 text-sm font-semibold text-zinc-900 dark:text-white">{{ data_get($customerLoyaltyApiStatus, 'configured_label') }}</p>
                    <p class="mt-1 break-all text-xs text-zinc-500 dark:text-zinc-400">{{ data_get($customerLoyaltyApiStatus, 'base_url') ?: '-' }}</p>
                </div>

                <div class="rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-950/50">
                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Tài khoản</p>
                    <p class="mt-2 text-sm font-semibold text-zinc-900 dark:text-white">{{ data_get($customerLoyaltyApiStatus, 'username') ?: '-' }}</p>
                    <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ data_get($customerLoyaltyApiStatus, 'has_password') ? 'Đã lưu mật khẩu' : 'Chưa có mật khẩu' }}</p>
                </div>

                <div class="rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-950/50">
                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Token</p>
                    <p class="mt-2 text-sm font-semibold {{ $apiTokenExpired ? 'text-amber-700 dark:text-amber-300' : 'text-zinc-900 dark:text-white' }}">{{ data_get($customerLoyaltyApiStatus, 'token_label') }}</p>
                    <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Hết hạn: {{ data_get($customerLoyaltyApiStatus, 'token_expires_at_label') ?: '-' }}</p>
                </div>

                <div class="rounded-2xl bg-zinc-50 p-4 dark:bg-zinc-950/50">
                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Gia hạn gần nhất</p>
                    <p class="mt-2 text-sm font-semibold text-zinc-900 dark:text-white">{{ data_get($customerLoyaltyApiStatus, 'token_refreshed_at_label') ?: '-' }}</p>
                    <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Tự đăng nhập lại khi token hết hạn hoặc API trả 401.</p>
                </div>
            </div>

            @if ($customerLoyaltyApiCheckResult)
                <div class="mt-4 rounded-2xl border px-4 py-3 text-sm {{ $apiCheckSucceeded ? 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300' : 'border-red-200 bg-red-50 text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300' }}">
                    <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                        <p class="font-semibold">{{ data_get($customerLoyaltyApiCheckResult, 'message') }}</p>
                        <p class="text-xs opacity-80">{{ data_get($customerLoyaltyApiCheckResult, 'checked_at_label') }}</p>
                    </div>

                    @if ($apiCheckSucceeded)
                        <p class="mt-2 text-xs opacity-80">Token: {{ data_get($customerLoyaltyApiCheckResult, 'token_preview') }}</p>
                    @endif
                </div>
            @endif
        </section>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Google reCAPTCHA v3</h2>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">Bật/tắt captcha cho các form public như tư vấn tour và đổi quà điểm thưởng.</p>
                    </div>

                    <label class="inline-flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-semibold text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
                        <input type="checkbox" wire:model.defer="form.google_recaptcha_v3_enabled" class="rounded border-zinc-300 text-red-600 focus:ring-red-500 dark:border-zinc-700 dark:bg-zinc-900">
                        Bật reCAPTCHA v3
                    </label>
                </div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Site key</label>
                        <input type="text" wire:model.defer="form.google_recaptcha_v3_site_key" autocomplete="off" placeholder="6Lc..." class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <p class="text-xs text-zinc-500 dark:text-zinc-400">Key public dùng để render Google reCAPTCHA v3 ở frontsite.</p>
                        @error('form.google_recaptcha_v3_site_key') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Secret key</label>
                        <input type="password" wire:model.defer="form.google_recaptcha_v3_secret_key" autocomplete="new-password" placeholder="Để trống nếu không đổi" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <p class="text-xs text-zinc-500 dark:text-zinc-400">Secret key được lưu dạng mã hóa và dùng để gọi API verify của Google.</p>
                        @error('form.google_recaptcha_v3_secret_key') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Ngưỡng điểm tối thiểu</label>
                        <input type="number" min="0" max="1" step="0.1" wire:model.defer="form.google_recaptcha_v3_min_score" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <p class="text-xs text-zinc-500 dark:text-zinc-400">Google trả score từ 0 đến 1. Mặc định 0.5 để cân bằng chống spam và tránh chặn khách thật.</p>
                        @error('form.google_recaptcha_v3_min_score') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="rounded-2xl bg-zinc-50 p-4 text-sm leading-6 text-zinc-600 dark:bg-zinc-950/50 dark:text-zinc-300">
                        <p class="font-semibold text-zinc-900 dark:text-white">Form đang áp dụng</p>
                        <p class="mt-2">Form tư vấn tour, form CTA cuối trang và form đổi quà điểm thưởng. Khi tắt, các form gửi bình thường và không gọi Google.</p>
                    </div>
                </div>
        </section>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                @php
                    $footerSocialKey = \App\Support\FooterSocialLinks::STRUCTURED_DATA_KEY;
                    $footerSocialPlatforms = \App\Support\FooterSocialLinks::platforms();
                    $footerSocialPlatformOptions = \App\Support\FooterSocialLinks::platformOptions();
                @endphp

                <div class="mt-6 rounded-3xl border border-zinc-200 bg-zinc-50/70 p-5 dark:border-zinc-800 dark:bg-zinc-950/40">
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <h3 class="text-base font-semibold text-zinc-900 dark:text-white">Social footer</h3>
                            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cấu hình các nút social hiển thị ở footer. Icon được khóa theo nền tảng bằng bộ icon chính hãng; màu hiển thị ngoài frontsite lấy theo theme Hải Đăng Travel.</p>
                        </div>

                        <button type="button" wire:click="addFooterSocialLink" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 px-4 py-2.5 text-sm font-semibold text-red-600 transition hover:bg-red-50 dark:border-red-500/30 dark:text-red-300 dark:hover:bg-red-500/10">
                    ...
                    <div class="mt-5 space-y-4">
                        @forelse (($form['structured_data'][$footerSocialKey] ?? []) as $index => $item)
                            @php
                                $platformKey = $item['platform'] ?? 'facebook';
                                $platform = $footerSocialPlatforms[$platformKey] ?? $footerSocialPlatforms['facebook'];
                            @endphp

                            <div wire:key="footer-social-link-{{ $index }}" class="rounded-3xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
                                <div class="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                    <div class="flex items-center gap-3">
                                        <span class="inline-flex size-11 shrink-0 items-center justify-center rounded-2xl bg-red-50 text-lg text-red-600 dark:bg-red-500/10 dark:text-red-300">
                                            @if (! empty($platform['image']))
                                                <img src="{{ asset($platform['image']) }}" alt="" class="h-7 w-auto" loading="lazy" aria-hidden="true">
                                            @else
                                                <i class="{{ $platform['icon'] ?? 'fa-brands fa-facebook-f' }}" aria-hidden="true"></i>
                                            @endif
                                        </span>
                                        <div>
                                            <p class="text-sm font-semibold text-zinc-900 dark:text-white">Kênh social {{ $loop->iteration }}</p>
                                            <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ $platform['label'] ?? 'Social' }}</p>
                                        </div>
                                    </div>

                                    <div class="flex flex-wrap items-center gap-2">
                                        <button type="button" wire:click="moveFooterSocialLink({{ $index }}, 'up')" @disabled($loop->first) class="rounded-2xl border border-zinc-200 px-3 py-2 text-sm font-medium text-zinc-600 transition hover:border-zinc-300 hover:text-zinc-900 disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-300 dark:hover:text-white">
                                            <i class="fa-solid fa-arrow-up"></i>
                                        </button>
                                        <button type="button" wire:click="moveFooterSocialLink({{ $index }}, 'down')" @disabled($loop->last) class="rounded-2xl border border-zinc-200 px-3 py-2 text-sm font-medium text-zinc-600 transition hover:border-zinc-300 hover:text-zinc-900 disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-300 dark:hover:text-white">
                                            <i class="fa-solid fa-arrow-down"></i>
                                        </button>
                                        <button type="button" wire:click="removeFooterSocialLink({{ $index }})" class="rounded-2xl border border-red-200 px-3 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50 dark:border-red-500/30 dark:text-red-300 dark:hover:bg-red-500/10">
                                            Xóa
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div class="space-y-2">
                                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Nền tảng</label>
                                        <select wire:model.defer="form.structured_data.{{ $footerSocialKey }}.{{ $index }}.platform" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                            @foreach ($footerSocialPlatformOptions as $option)
                                                <option value="{{ $option['value'] }}">{{ $option['label'] }}</option>
                                            @endforeach
                                        </select>
                                        @error("form.structured_data.$footerSocialKey.$index.platform") <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>

                                    <div class="space-y-2">
                                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">URL</label>
                                        <input type="url" wire:model.defer="form.structured_data.{{ $footerSocialKey }}.{{ $index }}.url" placeholder="https://..." class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                        @error("form.structured_data.$footerSocialKey.$index.url") <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>

                                    <div class="space-y-2">
                                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Nhãn hiển thị</label>
                                        <input type="text" wire:model.defer="form.structured_data.{{ $footerSocialKey }}.{{ $index }}.label" placeholder="Ví dụ: Fanpage chính thức" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                        @error("form.structured_data.$footerSocialKey.$index.label") <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>

                                    <div class="space-y-2">
                                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Mô tả ngắn</label>
                                        <input type="text" wire:model.defer="form.structured_data.{{ $footerSocialKey }}.{{ $index }}.description" placeholder="Ví dụ: Hỗ trợ khách hàng" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                        @error("form.structured_data.$footerSocialKey.$index.description") <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>

                                    <label class="inline-flex items-center gap-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                        <input type="checkbox" wire:model.defer="form.structured_data.{{ $footerSocialKey }}.{{ $index }}.is_active" class="rounded border-zinc-300 text-red-600 focus:ring-red-500 dark:border-zinc-700 dark:bg-zinc-900">
                                        Hiển thị ở footer
                                    </label>
                                </div>
                            </div>
                        @empty
                            <div class="rounded-3xl border border-dashed border-zinc-300 bg-white p-5 text-sm text-zinc-500 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400">
                                Chưa có kênh social footer nào. Bấm “Thêm kênh social” để tạo nút mới.
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Tên pháp lý doanh nghiệp</label>
                        <input type="text" wire:model.defer="form.structured_data.company.legal_name" placeholder="Công ty TNHH Du lịch Hải Đăng" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <p class="text-xs text-zinc-500 dark:text-zinc-400">Nguồn dùng cho `Organization.legalName` và các thông tin pháp lý hiển thị ngoài frontsite.</p>
                        @error('form.structured_data.company.legal_name') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Giấy phép ĐKKD</label>
                        <input type="text" wire:model.defer="form.structured_data.company.business_license" placeholder="Ví dụ: 0312345678" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        @error('form.structured_data.company.business_license') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Giấy phép lữ hành quốc tế</label>
                        <input type="text" wire:model.defer="form.structured_data.company.international_travel_license" placeholder="79-723/2017/TCDL-GP LHQT" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        @error('form.structured_data.company.international_travel_license') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Mã số thuế</label>
                        <input type="text" wire:model.defer="form.structured_data.company.tax_code" placeholder="Ví dụ: 0312345678" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        @error('form.structured_data.company.tax_code') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <input type="text" wire:model.defer="form.structured_data.address.address_country" placeholder="Ví dụ: VN" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 focus:bg-white dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <p class="text-xs text-zinc-500 dark:text-zinc-400">Các field địa chỉ này được dùng cho `PostalAddress` của `Organization` và `LocalBusiness` ngoài frontsite; quốc gia nên nhập mã ISO 2 ký tự, ví dụ `VN`.</p>
