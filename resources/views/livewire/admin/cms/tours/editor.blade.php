<div class="space-y-4">
    <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if ($editingTour && config('travel_reviews.enabled', true))
    ...
    <section class="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <form wire:submit.prevent="saveTour" class="space-y-5 pb-32 md:pb-6" data-admin-feedback-form data-admin-loading-text="Đang lưu tour...">
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <select wire:model.live="form.scope" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                ...
                <div class="md:col-span-2 xl:col-span-4 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    @if (config('travel_reviews.enabled', true))
                        Tổng điểm và tổng lượt đánh giá ở đây dùng để tạo aggregate rating ảo cho SEO/schema. Các review item chi tiết được quản lý ở màn <span class="font-semibold text-zinc-900 dark:text-white">Quản lý đánh giá</span> khi tính năng review được bật.
                    @else
                        Tổng điểm và tổng lượt đánh giá ở đây dùng để tạo aggregate rating ảo cho SEO/schema, không phụ thuộc trạng thái tắt/bật của review item chi tiết.
                    @endif
                </div>

                @if (config('travel_reviews.enabled', true))
                    <div class="md:col-span-2 xl:col-span-4 space-y-4 rounded-[24px] border border-amber-200 bg-amber-50/70 p-4 dark:border-amber-500/30 dark:bg-amber-500/10">
                        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            <div>
                                <h2 class="text-base font-semibold text-zinc-900 dark:text-white">Lượt đánh giá theo tour - ngày khởi hành</h2>
                                <p class="mt-1 text-sm leading-6 text-zinc-600 dark:text-zinc-300">
                                    Admin tạo từng lượt đánh giá cho khách quét QR. Mật khẩu có thể để trống; review gửi từ QR hoặc form public đều vào trạng thái draft để duyệt.
                                </p>
                            </div>

                            <button type="button" wire:click="addReviewBatch" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-amber-300 bg-white px-4 py-2 text-sm font-semibold text-amber-700 transition hover:border-amber-400 dark:border-amber-500/40 dark:bg-zinc-900 dark:text-amber-300">
                                <i class="fa-solid fa-plus"></i>
                                Thêm lượt
                            </button>
                        </div>

                        @if (($form['review_batches'] ?? []) === [])
                            <div class="rounded-2xl border border-dashed border-amber-200 bg-white px-4 py-5 text-sm text-zinc-600 dark:border-amber-500/30 dark:bg-zinc-900 dark:text-zinc-300">
                                Chưa có lượt đánh giá. Bấm “Thêm lượt” để tạo QR theo ngày khởi hành.
                            </div>
                        @endif

                        <div class="space-y-4">
                            @foreach (($form['review_batches'] ?? []) as $batchIndex => $reviewBatch)
                                @php
                                    $reviewSubmissionBatch = collect($reviewSubmission['batches'] ?? [])->firstWhere('id', (int) ($reviewBatch['id'] ?? 0));
                                @endphp

                                <div wire:key="tour-review-batch-{{ $batchIndex }}-{{ $reviewBatch['id'] ?? 'new' }}" class="grid gap-4 rounded-3xl border border-amber-200 bg-white p-5 shadow-sm dark:border-amber-500/30 dark:bg-zinc-900 lg:grid-cols-[1fr,210px]">
                                    <div class="space-y-4">
                                        <div class="flex items-center justify-between gap-3">
                                            <p class="text-xs font-semibold uppercase tracking-[0.22em] text-amber-700 dark:text-amber-300">Lượt {{ $loop->iteration }}</p>

                                            <button type="button" wire:click="removeReviewBatch({{ $batchIndex }})" class="rounded-2xl border border-red-200 px-3 py-2 text-sm font-medium text-red-600 dark:border-red-500/30 dark:text-red-300">
                                                Xóa
                                            </button>
                                        </div>

                                        <input type="hidden" wire:model.defer="form.review_batches.{{ $batchIndex }}.id">
                                        <input type="hidden" wire:model.defer="form.review_batches.{{ $batchIndex }}.token">

                                        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                                            <input type="text" wire:model.defer="form.review_batches.{{ $batchIndex }}.label" placeholder="Tên lượt, ví dụ: Đoàn Hà Nội 20/06" class="xl:col-span-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-amber-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">

                                            <select wire:model.defer="form.review_batches.{{ $batchIndex }}.tour_departure_id" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-amber-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                                <option value="">Không gắn departure</option>
                                                @foreach (($editingTour?->departures ?? collect()) as $departureOption)
                                                    <option value="{{ $departureOption->id }}">
                                                        {{ optional($departureOption->departure_date)->format('d/m/Y') ?: 'Chưa có ngày' }}{{ $departureOption->standard_label ? ' - '.$departureOption->standard_label : '' }}
                                                    </option>
                                                @endforeach
                                            </select>

                                            <input type="date" wire:model.defer="form.review_batches.{{ $batchIndex }}.departure_date" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-amber-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                            <input type="number" wire:model.defer="form.review_batches.{{ $batchIndex }}.sort_order" placeholder="Thứ tự" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-amber-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                            <input type="text" wire:model.defer="form.review_batches.{{ $batchIndex }}.password" autocomplete="off" placeholder="Mật khẩu tùy chọn" class="xl:col-span-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-amber-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">

                                            <label class="xl:col-span-2 flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-zinc-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-zinc-200">
                                                <input type="checkbox" wire:model.defer="form.review_batches.{{ $batchIndex }}.enabled" class="rounded border-zinc-300 text-amber-600 focus:ring-amber-500">
                                                Bật QR/form cho lượt này
                                            </label>
                                        </div>
                                    </div>

                                    <div class="rounded-[22px] border border-zinc-100 bg-zinc-50 p-4 text-center dark:border-zinc-800 dark:bg-zinc-950/50">
                                        @if (($reviewSubmissionBatch['enabled'] ?? false) && filled($reviewSubmissionBatch['qr_svg'] ?? ''))
                                            <div class="mx-auto inline-flex rounded-2xl bg-white p-3">
                                                {!! $reviewSubmissionBatch['qr_svg'] !!}
                                            </div>
                                            <p class="mt-2 text-xs font-semibold uppercase tracking-[0.16em] text-amber-700 dark:text-amber-300">QR đánh giá</p>
                                            <p class="mt-2 break-all font-mono text-[11px] leading-5 text-zinc-500 dark:text-zinc-400">{{ $reviewSubmissionBatch['url'] }}</p>
                                            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">{{ ($reviewSubmissionBatch['requires_password'] ?? false) ? 'Có mật khẩu' : 'Không dùng mật khẩu' }}</p>
                                        @else
                                            ...
                                            <p class="mt-3 text-sm font-medium text-zinc-600 dark:text-zinc-300">Lưu tour để tạo QR.</p>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
