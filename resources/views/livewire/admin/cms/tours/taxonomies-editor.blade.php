<div class="space-y-4">
    <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if ($editingTaxonomyModel && config('travel_reviews.enabled', true) && filled($taxonomyConfig['reviews_index_route'] ?? null))
    ...
    <section class="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <form wire:submit.prevent="{{ $taxonomyConfig['save_action'] }}" class="space-y-5 pb-32 md:pb-6" data-admin-feedback-form data-admin-loading-text="Đang lưu {{ $taxonomyConfig['singular_label'] }}...">
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                @if ($taxonomyConfig['type'] === 'destination')
                    <label class="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
                        <input type="checkbox" wire:model.live="{{ $formPath }}.is_country_root" class="rounded border-zinc-300 text-red-600 focus:ring-red-500">
                        Là quốc gia root
                    </label>

                    @if (! ($taxonomyForm['is_country_root'] ?? false))
                        <div class="space-y-2">
                            <select wire:model.defer="{{ $formPath }}.country_id" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                                <option value="">Quốc gia</option>
                                @foreach ($countryRootOptions as $countryRoot)
                                    @if (! $editingTaxonomyModel || (int) $editingTaxonomyModel->getKey() !== (int) $countryRoot->getKey())
                                        <option value="{{ $countryRoot->id }}">{{ $countryRoot->name }}</option>
                                    @endif
                                @endforeach
                            </select>
                            @error($formPath.'.country_id')
                    ...
                    <label class="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
                        <input type="checkbox" wire:model.defer="{{ $formPath }}.show_tours_on_page" class="rounded border-zinc-300 text-red-600 focus:ring-red-500">
                        Hiển thị tour trên trang điểm đến
                    </label>

                    <label class="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
                        <input type="checkbox" wire:model.defer="{{ $formPath }}.show_blogs_on_page" class="rounded border-zinc-300 text-red-600 focus:ring-red-500">
                        Hiển thị blog trên trang điểm đến
                    </label>
                @endif
            ...
            <x-admin.image-dropzone
                label="Ảnh hero / avatar"
                model="avatarUpload"
                :preview="$avatarPreview"
                hint="Ảnh này dùng làm ảnh hero của trang taxonomy và ảnh đại diện trong các rail/card liên quan."
            />

            <div class="grid gap-4 md:grid-cols-2">
                <input type="text" wire:model.defer="{{ $formPath }}.cover_alt" placeholder="Alt text ảnh hero" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                <input type="url" wire:model.defer="{{ $formPath }}.cover_image_url" placeholder="URL ảnh ngoài cho hero/avatar" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button
                    ...
                    data-button-label="Chọn ảnh này làm hero/avatar"
            ...
            <x-admin.quill-editor wire:key="{{ $taxonomyConfig['type'] }}-excerpt-{{ $editingTaxonomyModel?->id ?? 'new' }}-{{ md5((string) ($taxonomyForm['excerpt'] ?? '')) }}" model="{{ $formPath }}.excerpt" :value="$taxonomyForm['excerpt'] ?? ''" rows="3" placeholder="Tóm tắt ngắn hiển thị trong hero" />
            <x-admin.quill-editor wire:key="{{ $taxonomyConfig['type'] }}-content-{{ $editingTaxonomyModel?->id ?? 'new' }}-{{ md5((string) ($taxonomyForm['content'] ?? '')) }}" model="{{ $formPath }}.content" :value="$taxonomyForm['content'] ?? ''" mode="rich" :allow-images="true" rows="12" placeholder="Nội dung hiển thị dưới hero trước danh sách tour" />
            ...
            @if (in_array($taxonomyConfig['type'], ['category', 'destination'], true))
                <div class="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    Tổng điểm và tổng lượt đánh giá ở đây dùng cho aggregate rating ảo của trang landing. Review item chi tiết được quản lý ở màn <span class="font-semibold text-zinc-900 dark:text-white">Quản lý đánh giá</span> khi tính năng review được bật.
                </div>
            @endif
