        ...
        <form wire:submit="savePost" class="space-y-5 pb-32 md:pb-6" data-admin-feedback-form data-admin-loading-text="Đang lưu bài viết blog...">
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-2">
                    ...
                    <select wire:model.live="form.country_destination_id" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <option value="">Không chọn</option>
                        @foreach ($countryRootOptions as $countryRoot)
                            <option value="{{ $countryRoot->id }}">{{ $countryRoot->name }}</option>
                        @endforeach
                    </select>
                    <p class="text-xs leading-5 text-zinc-500 dark:text-zinc-400">Quốc gia được lưu bằng chính bản ghi điểm đến root.</p>
                    @error('form.country_destination_id')
                        <p class="text-xs font-medium text-red-600 dark:text-red-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select wire:model.live="form.destination_id" class="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none transition focus:border-red-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white">
                        <option value="">Không chọn</option>
                        @foreach ($destinationOptions as $destinationOption)
                            <option value="{{ $destinationOption->id }}">
                                {{ $destinationOption->name }}@if ($destinationOption->country) - {{ $destinationOption->country->name }}@endif
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs leading-5 text-zinc-500 dark:text-zinc-400">Khi chọn điểm đến, hệ thống tự đồng bộ quốc gia tương ứng.</p>
