<section class="space-y-4 rounded-3xl border border-zinc-200 bg-zinc-50/70 p-5 dark:border-zinc-800 dark:bg-zinc-950/40">
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Gallery dự án</h3>
            <p class="text-sm text-zinc-500 dark:text-zinc-400">Cho phép thêm ảnh công trình hoặc video YouTube / MP4 để mở fullscreen ở trang chi tiết dự án.</p>
        </div>
    ...
    @if (empty($form['gallery'] ?? []))
        <div class="rounded-3xl border border-dashed border-zinc-300 bg-white/80 px-5 py-6 text-sm text-zinc-500 dark:border-zinc-700 dark:bg-zinc-900/60 dark:text-zinc-400">
            Chưa có gallery. Hãy thêm ảnh hiện trường hoặc video để tăng chiều sâu cho case study dự án.
        </div>
    @endif
    ...
    <div class="space-y-5">
        @foreach (($form['gallery'] ?? []) as $index => $item)
            @php
                $galleryCollection = \App\Support\ContentGallery::projectCollection($item['uuid']);
                $galleryUpload = $galleryUploads[$index] ?? null;
                $galleryPreview = $galleryUpload ? $galleryUpload->temporaryUrl() : (($selectedGalleryLibraryMedia[$item['uuid']] ?? null)?->getUrl() ?: $editingProject?->getFirstMediaUrl($galleryCollection));
                $isImage = ($item['type'] ?? 'image') === 'image';
            @endphp

            <div wire:key="project-gallery-item-{{ $item['uuid'] }}" class="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="mb-4 flex items-center justify-between gap-3">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.25em] text-red-600">Gallery item {{ $loop->iteration }}</p>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">Có thể là ảnh, video YouTube hoặc video MP4.</p>
                    </div>
                ...
                @if ($isImage)
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-3">
                        <button
                            ...
                            data-button-label="Chọn ảnh này cho gallery dự án"
