...
class Tour extends Model implements HasMedia
{
    ...
    protected function casts(): array
    {
        return [
            ...
            'review_submission_enabled' => 'boolean',
            'review_submission_password' => 'encrypted',
            'sale_price' => 'integer',
            'schema' => 'array',
    ...
    {
        return $this->hasMany(TourReviewBatch::class, 'tour_id')->ordered();
    }

    public function publicReviewUrl(): ?string
    {
        $reviewBatch = $this->relationLoaded('reviewBatches')
            ? $this->reviewBatches->first(fn (TourReviewBatch $batch): bool => (bool) $batch->enabled && filled($batch->token))
            : $this->reviewBatches()->enabled()->ordered()->first();

        if ($reviewBatch) {
            return route('tour-reviews.public.show', [
                'tour' => $this,
                'token' => $reviewBatch->token,
            ]);
        }

        if ($this->review_submission_token) {
            return route('tour-reviews.public.show', [
                'tour' => $this,
                'token' => $this->review_submission_token,
            ]);
        }
    ...
    public function acceptsPublicReviews(): bool
    {
        if ($this->relationLoaded('reviewBatches')) {
            return $this->reviewBatches->contains(fn (TourReviewBatch $batch): bool => (bool) $batch->enabled && filled($batch->token));
        }

        return $this->reviewBatches()->enabled()->whereNotNull('token')->exists()
            || ((bool) $this->review_submission_enabled && filled($this->review_submission_token));
    }
