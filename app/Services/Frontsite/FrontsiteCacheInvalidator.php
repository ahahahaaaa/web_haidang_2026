...
class FrontsiteCacheInvalidator
{
    ...
    public function groupsFor(Model $model): array
    {
        ...
        if ($model instanceof TourReviewBatch) {
            return array_filter([
                'home',
                'tours',
                filled($model->tour_id) ? 'tour:'.$model->tour_id : null,
        ...
        if ($model instanceof Destination) {
            return array_filter([
                ...
                (bool) $model->is_country_root ? 'countries' : null,
                (bool) $model->is_country_root ? 'country:'.$model->getKey() : null,
                filled($model->country_id) ? 'country:'.$model->country_id : null,
                'tours',
            ]);
        }
        ...
        if ($model instanceof BlogPost) {
            return array_filter([
                ...
                filled($model->country_destination_id) ? 'country:'.$model->country_destination_id : null,
                filled($model->destination_id) ? 'destination:'.$model->destination_id : null,
            ]);
        }
        ...
        if ($model instanceof TravelReview) {
            ...
            if ($model->reviewable_type === Tour::class && filled($model->reviewable_id)) {
                $reviewableGroups = ['tours', 'tour:'.$model->reviewable_id];
            }

            if ($model->reviewable_type === TourCategory::class && filled($model->reviewable_id)) {
                $reviewableGroups = ['tours', 'taxonomies', 'tour-categories', 'tour-category:'.$model->reviewable_id];
            }

            if ($model->reviewable_type === Destination::class && filled($model->reviewable_id)) {
                $reviewableGroups = ['tours', 'taxonomies', 'destinations', 'destination:'.$model->reviewable_id];
            }
