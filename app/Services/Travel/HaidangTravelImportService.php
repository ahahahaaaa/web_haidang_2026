...
class HaidangTravelImportService
{
    ...
    {
        $defaultCountryId = $this->ensureCountryRoot('Việt Nam', TourScope::Domestic->value);

        foreach ($destinations as $index => $destination) {
            $rawSlug = (string) data_get($destination, 'slug');
            $name = (string) data_get($destination, 'name');
            $isCountryRoot = (bool) data_get($destination, 'is_country_root', false)
                || in_array(Destination::countryBaseSlug($rawSlug), ['viet-nam', 'quoc-te'], true);
            $slug = $isCountryRoot ? Destination::countryRootSlug($rawSlug ?: $name) : $rawSlug;
            $countryId = $isCountryRoot
                ? null
                : $this->countryIdForDestinationPayload($destination, $defaultCountryId);
            $values = [
            ...
            if (Schema::hasColumn('destinations', 'show_tours_on_page')) {
                $values['show_tours_on_page'] = (bool) data_get($destination, 'show_tours_on_page', true);
            }

            if (Schema::hasColumn('destinations', 'show_blogs_on_page')) {
                $values['show_blogs_on_page'] = (bool) data_get($destination, 'show_blogs_on_page', false);
            }
    ...
    protected function importBlogPosts(array $posts): void
    {
        foreach ($posts as $post) {
            $destinationId = $this->destinationIdBySlug((string) data_get($post, 'destination_slug'));
            $countryId = $this->countryIdForBlogPostPayload($post, $destinationId);
    ...
    protected function countryIdForDestinationPayload(array $destination, ?int $fallbackCountryId): ?int
    {
        if (! Schema::hasColumn('destinations', 'country_id')) {
            return null;
        }

        $explicitSlug = trim((string) data_get($destination, 'country_slug'));
        $explicitName = trim((string) (data_get($destination, 'country_name') ?: data_get($destination, 'country')));

        if ($explicitSlug !== '' || $explicitName !== '') {
            return $this->ensureCountryRoot(
                $explicitName !== '' ? $explicitName : $this->countryNameFromSlug($explicitSlug),
                $this->normalizeScope(data_get($destination, 'scope'), $explicitSlug, $explicitName),
                $explicitSlug !== '' ? $explicitSlug : null,
            );
        }

        $inferredCountryName = $this->inferCountryNameForDestination($destination);

        return $this->ensureCountryRoot(
            $inferredCountryName,
            $inferredCountryName === 'Việt Nam' ? TourScope::Domestic->value : TourScope::International->value,
        ) ?: $fallbackCountryId;
    }

    protected function countryIdForBlogPostPayload(array $post, ?int $destinationId): ?int
    {
        if (! Schema::hasColumn('blog_posts', 'country_destination_id')) {
            return null;
        }

        $explicitSlug = trim((string) data_get($post, 'country_slug'));
        $explicitName = trim((string) (data_get($post, 'country_name') ?: data_get($post, 'country')));

        if ($explicitSlug !== '' || $explicitName !== '') {
            return $this->ensureCountryRoot(
                $explicitName !== '' ? $explicitName : $this->countryNameFromSlug($explicitSlug),
                $this->normalizeScope(null, $explicitSlug, $explicitName),
                $explicitSlug !== '' ? $explicitSlug : null,
            );
        }
    ...
    protected function ensureCountryRoot(string $name, ?string $scope = null, ?string $slug = null): ?int
    {
        if (! Schema::hasColumn('destinations', 'is_country_root')) {
            return null;
        }

        $name = trim($name) !== '' ? trim($name) : 'Việt Nam';
        $baseSlug = Destination::countryBaseSlug(trim((string) ($slug ?: $name)));
        $baseSlug = $baseSlug !== '' ? $baseSlug : 'viet-nam';
        $slug = Destination::countryRootSlug($baseSlug);
        $country = Destination::query()->where('slug', $slug)->first()
            ?: Destination::query()
                ->countryRoots()
                ->where('slug', $baseSlug)
                ->first();
        ...
        if ($country) {
            $country->forceFill([
                ...
                'status' => 'published',
                'published_at' => $country->published_at ?: now(),
        ...
        return (int) $country->getKey();
    }

    protected function inferCountryNameForDestination(array $destination): string
    {
        $haystack = Str::of(implode(' ', array_filter([
            data_get($destination, 'slug'),
            data_get($destination, 'name'),
            data_get($destination, 'region_slug'),
            data_get($destination, 'scope'),
        ])))
            ->ascii()
            ->lower()
        ...
        return TourScope::tryFrom((string) data_get($destination, 'scope')) === TourScope::International
            ? 'Quốc tế'
            : 'Việt Nam';
    }

    protected function countryNameFromSlug(string $slug): string
    {
        $slug = Destination::countryBaseSlug($slug);
        ...
        return match ($slug) {
            ...
            'uc' => 'Úc',
            'hoa-ky', 'my' => 'Hoa Kỳ',
            default => Str::of($slug)->replace('-', ' ')->title()->value(),
        };
    }
    ...
    {
        return $slug !== ''
            ? Destination::query()->regularDestinations()->where('slug', $slug)->value('id')
            : null;
    }
    ...
    {
        return $this->taxonomyIdsBySlug($slugs, Destination::query()->regularDestinations());
    }
