...
class HaidangTravelSnapshotCrawler
{
    ...
    protected function inferCountryName(?TourScope $scope, string $destinationName, ?string $regionName): string
    {
        $value = Str::of($destinationName.' '.$regionName)
            ->ascii()
            ->lower()
            ->value();
        $map = [
            'Việt Nam' => ['viet nam', 'mien bac', 'mien trung', 'mien nam', 'tay nguyen', 'ha noi', 'nha trang', 'ninh chu', 'vinh hy', 'binh hung', 'phu quoc', 'da lat'],
            'Trung Quốc' => ['trung quoc', 'thuong hai', 'bac kinh', 'le giang', 'shangrila', 'shangri la', 'dai ly', 'tay tang'],
            'Hàn Quốc' => ['han quoc', 'seoul', 'busan'],
            'Nhật Bản' => ['nhat ban', 'tokyo', 'osaka'],
            'Đài Loan' => ['dai loan', 'taiwan'],
            'Thái Lan' => ['thai lan', 'bangkok'],
            'Singapore' => ['singapore'],
            'Malaysia' => ['malaysia'],
        ...
        return $scope === TourScope::International ? 'Quốc tế' : 'Việt Nam';
    }
    ...
    protected function parseTourPage(string $url, array $context): ?array
    {
        ...
        $countryName = $this->inferCountryName($scope, $destinationName, $regionName);
        ...
        $countrySlug = Destination::countryRootSlug($countryName);
        ...
        return [
            ...
            'destination' => $this->destinationPayload($destinationName, $scope, $regionName, $countryName),
    ...
    protected function destinationPayload(string $name, ?TourScope $scope, ?string $regionName, string $countryName): array
    {
        return [
            'name' => $name,
            'slug' => Str::slug($name),
            ...
            'country_slug' => Destination::countryRootSlug($countryName),
            'region_slug' => $regionName ? Str::slug($regionName) : null,
            'status' => 'published',
            'sort_order' => 0,
        ];
    }
