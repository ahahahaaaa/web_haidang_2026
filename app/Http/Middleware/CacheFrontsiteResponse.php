...
class CacheFrontsiteResponse
{
    ...
    protected function groupsFor(Request $request): array
    {
        ...
        return match ($routeName) {
            ...
            'destinations.show' => [...$groups, 'tours', 'blog', 'taxonomies', 'destinations', 'countries'],
            'regions.show' => [...$groups, 'tours', 'taxonomies', 'regions', 'region:'.$this->routeModelKey($request, 'region')],
            'countries.show' => [...$groups, 'tours', 'taxonomies', 'countries'],
    ...
    protected function hasUnsafeCookies(SymfonyResponse $response): bool
    {
        $allowedCookieNames = array_filter([
            'XSRF-TOKEN',
            (string) config('session.cookie'),
        ]);

        foreach ($response->headers->getCookies() as $cookie) {
            if (! in_array($cookie->getName(), $allowedCookieNames, true)) {
                return true;
            }
        }
