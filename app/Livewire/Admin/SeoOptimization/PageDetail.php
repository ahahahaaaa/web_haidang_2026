...
class PageDetail extends OptimizationComponent
{
    ...
    public function saveBrief(OptimizationWorkflowService $workflow): void
    {
        ...
        $this->perform(fn () => $workflow->saveBrief($page, $brief, $this->actor()), 'Đã lưu brief từ khóa. Nội dung public chưa thay đổi.');
    }

    public function runAudit(OptimizationWorkflowService $workflow): void
    {
        $page = $this->page('audit');
        $this->perform(fn () => $workflow->audit($page, $this->actor()), 'Đã cập nhật kết quả kiểm tra SEO. Xem phạm vi đánh giá và dữ liệu còn thiếu bên dưới.');
    }

    public function enqueue(OptimizationWorkflowService $workflow): void
    {
        $page = $this->page('propose');
        $this->perform(fn () => $workflow->enqueue($page, $this->actor(), $this->requestKey), 'Đã đưa vào hàng chờ Codex. Kết quả sẽ là đề xuất chờ người duyệt.');
    }
    ...
    public function render()
    {
        ...
        return view('livewire.admin.seo-optimization.page-detail', [
            ...
            'proposals' => $page->proposals()->latest()->limit(20)->get(),
