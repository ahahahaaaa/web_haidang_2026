...
class LandingPagesManager extends Component
{
    ...
    public function addBlock(string $type): void
    {
        ...
        if (! array_key_exists($type, $this->blockTypesForCms())) {
            return;
        }
        ...
        $this->syncHomeLayoutOrderToForm();
    }
    ...
    public function addVoucherPromotionBenefit(int $blockIndex): void
    {
        $this->authorizeAdminPermission('admin.landing-pages.edit');

        $benefits = data_get($this->form, 'blocks.'.$blockIndex.'.benefits', []);

        if (! is_array($benefits)) {
            $benefits = [];
        }

        $benefits[] = LandingPageBlocks::defaultVoucherPromotionBenefit();
        data_set($this->form, 'blocks.'.$blockIndex.'.benefits', array_values($benefits));
    }

    public function addVoucherPromotionStep(int $blockIndex): void
    {
        $this->authorizeAdminPermission('admin.landing-pages.edit');

        $steps = data_get($this->form, 'blocks.'.$blockIndex.'.steps', []);

        if (! is_array($steps)) {
            $steps = [];
        }

        $steps[] = LandingPageBlocks::defaultVoucherPromotionStep();
    ...
    {
        $this->moveHomeLayoutItemDown(TravelHomePageConfig::homeLayoutTokenForSection($sectionKey));
    }

    public function moveHomeSectionUp(string $sectionKey): void
    {
        $this->moveHomeLayoutItemUp(TravelHomePageConfig::homeLayoutTokenForSection($sectionKey));
    }

    public function moveHomeLayoutItemDown(string $token): void
    {
        $this->authorizeAdminPermission('admin.landing-pages.edit');

        $this->syncHtmlDraftsToFormBlocks();
        $order = $this->editableHomeLayoutOrder();
        $index = array_search($token, $order, true);
    ...
    public function moveHomeLayoutItemUp(string $token): void
    {
        $this->authorizeAdminPermission('admin.landing-pages.edit');
    ...
    public function removeVoucherPromotionBenefit(int $blockIndex, int $benefitIndex): void
    {
        $this->authorizeAdminPermission('admin.landing-pages.edit');

        $benefits = data_get($this->form, 'blocks.'.$blockIndex.'.benefits', []);

        if (! is_array($benefits) || ! array_key_exists($benefitIndex, $benefits)) {
            return;
        }

        array_splice($benefits, $benefitIndex, 1);
        data_set(
            $this->form,
            'blocks.'.$blockIndex.'.benefits',
            $benefits === [] ? [LandingPageBlocks::defaultVoucherPromotionBenefit()] : array_values($benefits),
        );
    }

    public function removeVoucherPromotionStep(int $blockIndex, int $stepIndex): void
    {
        $this->authorizeAdminPermission('admin.landing-pages.edit');

        $steps = data_get($this->form, 'blocks.'.$blockIndex.'.steps', []);

        if (! is_array($steps) || ! array_key_exists($stepIndex, $steps)) {
            return;
        }
        ...
        data_set(
            $this->form,
            'blocks.'.$blockIndex.'.steps',
            $steps === [] ? [LandingPageBlocks::defaultVoucherPromotionStep()] : array_values($steps),
        );
    }
    ...
    public function render()
    {
        ...
        return view($this->resolveView(), [
            ...
            'homeSectionOrder' => TravelHomePageConfig::sectionOrder($this->form['home_config'] ?? []),
        ]);
    }
     ...
     * @return array<string, string>
     */
    protected function homePositionOptionsForCms(): array
    {
        $positions = LandingPageBlocks::homePositionOptions();

        if (! $this->geoCmsEnabled()) {
            unset($positions[LandingPageBlocks::HOME_POSITION_BEFORE_GEO_ANSWER]);
        }
     ...
     */
    protected function editableFormBlocks(): array
    {
        return collect($this->form['blocks'] ?? [])
     ...
     */
    protected function editableHomeSectionOrder(): array
    {
        return TravelHomePageConfig::sectionOrder($this->form['home_config'] ?? []);
    }

    /**
     * @return array<int, string>
     */
    protected function editableHomeLayoutOrder(): array
    {
        return TravelHomePageConfig::homeLayoutOrder(
            $this->form['home_config'] ?? [],
            $this->form['blocks'] ?? [],
        );
    }
     ...
     * @return array<int, array<string, mixed>>
     */
    protected function homeLayoutItemsForAdmin(): array
    {
        $blockTypes = $this->blockTypesForCms();
        $blocks = collect(LandingPageBlocks::normalize($this->form['blocks'] ?? []))->values();
        $blocksByUuid = $blocks->keyBy(fn (array $block) => (string) ($block['uuid'] ?? ''));
        $blockIndexByUuid = $blocks
            ->mapWithKeys(fn (array $block, int $index) => [(string) ($block['uuid'] ?? '') => $index])
            ->filter(fn ($index, string $uuid) => $uuid !== '');
        $sectionOptions = $this->homeSectionOptionsForCms();

        return collect($this->editableHomeLayoutOrder())
    ...
    {
        $order = TravelHomePageConfig::homeLayoutOrder(
            ['layout_order' => $order, 'section_order' => data_get($this->form, 'home_config.section_order', [])],
            $this->form['blocks'] ?? [],
        );

        data_set($this->form, 'home_config.layout_order', $order);
        data_set($this->form, 'home_config.section_order', TravelHomePageConfig::sectionOrderFromLayout($order));
    }

    protected function syncHomeLayoutOrderToForm(): void
    {
        $this->setHomeLayoutOrder(data_get($this->form, 'home_config.layout_order', []));
    }

    protected function moveHomeBlockToSelectedPosition(int $blockIndex): void
    {
        if (($this->form['page_key'] ?? null) !== 'home') {
            return;
        }

        $block = data_get($this->form, 'blocks.'.$blockIndex);

        if (! is_array($block) || blank($block['uuid'] ?? null)) {
            return;
        }

        $token = TravelHomePageConfig::homeLayoutTokenForBlock((string) $block['uuid']);
        $order = collect(data_get($this->form, 'home_config.layout_order', $this->editableHomeLayoutOrder()))
            ->map(fn (mixed $item) => trim((string) $item))
            ->reject(fn (string $item) => $item === $token)
            ->values()
            ->all();
    ...
    protected function syncHtmlDraftsFromBlocks(?array $blocks = null): void
    {
        $drafts = [];

        foreach (($blocks ?? ($this->form['blocks'] ?? [])) as $block) {
            if (! is_array($block) || ($block['type'] ?? null) !== LandingPageBlocks::TYPE_HTML_WIDGET) {
                continue;
            }
            ...
            $drafts[$uuid] = array_key_exists($uuid, $this->blockHtmlDrafts)
                ? (string) $this->blockHtmlDrafts[$uuid]
                : (string) ($block['html'] ?? '');
        }

        $this->blockHtmlDrafts = $drafts;
    }

    protected function syncHtmlDraftsToFormBlocks(): void
    {
        $blocks = $this->form['blocks'] ?? [];
        ...
        foreach ($blocks as $index => $block) {
            ...
            if ($uuid === '' || ! array_key_exists($uuid, $this->blockHtmlDrafts)) {
                continue;
            }

            $blocks[$index]['html'] = (string) $this->blockHtmlDrafts[$uuid];
        }
    ...
    protected function duplicateBlockData(array $block): array
    {
        ...
        if (in_array($duplicate['type'] ?? null, [
            LandingPageBlocks::TYPE_VOUCHER_PROMOTION,
            LandingPageBlocks::TYPE_VOUCHER_PROMOTION_PREMIUM,
        ], true)) {
            $duplicate['benefits'] = collect($duplicate['benefits'] ?? [])
                ->map(function (array $benefit): array {
            ...
            $duplicate['steps'] = collect($duplicate['steps'] ?? [])
    ...
    protected function formFromModel(LandingPage $page): array
    {
        ...
        return [
            ...
            'geo_config' => $this->defaultGeoConfigForm($page->geo_config),
            'html_content' => $page->isHtmlMode() ? (string) ($page->body ?? '') : '',
            'home_config' => $this->normalizedHomeConfig($page->home_config, [], $blocks),
            'blocks' => $blocks,
        ];
    }
    ...
    protected function resetEditorState(): void
    {
        ...
        $this->form['home_config'] = $this->normalizedHomeConfig(null, [], $this->form['blocks']);
        $this->syncHtmlDraftsFromBlocks();
    }
    ...
    protected function rules(): array
    {
        return [
            ...
            'form.home_config.section_order' => ['nullable', 'array'],
            'form.home_config.section_order.*' => ['nullable', 'string', Rule::in(array_keys(TravelHomePageConfig::homeSectionOptions()))],
            'form.home_config.layout_order' => ['nullable', 'array'],
            'form.home_config.layout_order.*' => ['nullable', 'string', 'max:160'],
            ...
            'form.home_config.geo_answer' => ['nullable', 'array'],
            ...
            'form.home_config.region_taxonomy_tabs.is_enabled' => ['boolean'],
            ...
            'form.home_config.gallery' => ['nullable', 'array'],
            'form.home_config.gallery.is_enabled' => ['boolean'],
            ...
            'form.home_config.faq' => ['nullable', 'array'],
            'form.home_config.faq.is_enabled' => ['boolean'],
            'form.home_config.cta' => ['nullable', 'array'],
            'form.home_config.cta.is_enabled' => ['boolean'],
            'form.blocks' => ['array'],
            ...
            'form.blocks.*.tag_label' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.kicker' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.title_prefix' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.title_highlight' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.title_suffix' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.offer_label' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.offer_code' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.offer_note' => ['nullable', 'string', 'max:500'],
            'form.blocks.*.show_countdown' => ['boolean'],
            'form.blocks.*.countdown_label' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.countdown_expired_label' => ['nullable', 'string', 'max:180'],
            'form.blocks.*.panel_eyebrow' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.panel_title' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.panel_description' => ['nullable', 'string', 'max:500'],
            'form.blocks.*.primary_label' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.secondary_label' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.secondary_url' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.voucher_campaign_slug' => ['nullable', 'string', 'max:120'],
            'form.blocks.*.inquiry_context' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.inquiry_subject' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.modal_title' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.modal_description' => ['nullable', 'string', 'max:500'],
            'form.blocks.*.trust_note' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.benefits' => ['nullable', 'array', 'min:1', 'max:8'],
            'form.blocks.*.benefits.*.uuid' => ['nullable', 'string', 'max:100'],
            'form.blocks.*.benefits.*.icon' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.benefits.*.text' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.steps' => ['nullable', 'array', 'min:1', 'max:6'],
            'form.blocks.*.steps.*.uuid' => ['nullable', 'string', 'max:100'],
            'form.blocks.*.steps.*.title' => ['nullable', 'string', 'max:255'],
            'form.blocks.*.steps.*.text' => ['nullable', 'string', 'max:500'],
            'form.blocks.*.answer_summary' => ['nullable', 'string', 'max:600'],
