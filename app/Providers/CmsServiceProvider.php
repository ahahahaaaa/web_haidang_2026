...
class CmsServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        View::composer('themes.*', function ($view): void {
            ...
            $googleRecaptchaV3SiteKey = trim((string) ($settings->google_recaptcha_v3_site_key ?: config('services.google_recaptcha_v3.site_key')));
            $googleRecaptchaV3Enabled = (bool) $settings->google_recaptcha_v3_enabled && $googleRecaptchaV3SiteKey !== '';
            ...
            $view->with([
                ...
                'googleRecaptchaV3Enabled' => $googleRecaptchaV3Enabled,
    ...
    {
        View::composer('themes.haidangtravel.pages.errors.404', function ($view): void {
            $hotTours = $this->frontsiteSuggestedTours(featuredOnly: true, limit: 4);
            $latestTours = $this->frontsiteSuggestedTours(
                featuredOnly: false,
                limit: 4,
                excludeIds: $hotTours->pluck('id')->all(),
            );
     ...
     */
    protected function frontsiteSuggestedTours(bool $featuredOnly, int $limit, array $excludeIds = []): Collection
    {
        ...
        if (Schema::hasTable('tour_departures')) {
            $relations['departures'] = fn ($query) => $query->published()->orderBy('departure_date');
        }
        ...
        if (Schema::hasTable('tour_categories')) {
            $relations[] = 'primaryCategory.media';
        }

        if (Schema::hasTable('regions')) {
            $relations[] = 'region.media';
        }
